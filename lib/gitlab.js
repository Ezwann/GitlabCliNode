...
class GitLab {
    ...
    async issuesStats(created_after, created_before) {
        ...
        var res = await querier(`${this.url}projects/${this.project_id}/issues_statistics?access_token=${this.api_key}&created_after=${created_after.toString()}&created_before=${created_before.toString()}&scope=all`)
        // console.log(res.data.statistics.counts.all);
        return res.data.statistics.counts.all;
    }

    async listRecentIssues() {
        await this.checkInit();
        var res = await querier(`${this.url}projects/${this.project_id}/issues?access_token=${this.api_key}`);
    ...
    async listClosedIssues(created_after, created_before) {
        await this.checkInit();
        var res = await querier(`${this.url}projects/${this.project_id}/issues?&state=closed&access_token=${this.api_key}&created_after=${created_after.toString()}&created_before=${created_before.toString()}`);
        // console.log(res.data);
        return res.data;
    }
    ...
    async listOpenedIssues(created_after, created_before) {
        await this.checkInit();
        var res = await querier(`${this.url}projects/${this.project_id}/issues?&state=opened&access_token=${this.api_key}&created_after=${created_after.toString()}&created_before=${created_before.toString()}`);
        // console.log(res.data);
        return res.data;
